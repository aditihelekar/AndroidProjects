...
public class CityWeatherActivity extends AppCompatActivity {
    ...
    public static String Temp_Metric_Celcius = "temp_metric";
    public static String API_KEY = "MRh3DLhCcBY9778T48NGaLI2AaMJxZd4";
    private String currentCity, currentCountry, token;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null && getIntent().getExtras().get("Country") != null && getIntent().getExtras().get("CityName") != null) {
            ...
            currentCity = getIntent().getExtras().get("CityName").toString();
            currentCountry = getIntent().getExtras().get("Country").toString();
            preferences = getSharedPreferences(PREFS_NAME, 0);
            final boolean isCelciusMetric = preferences.getBoolean(Temp_Metric_Celcius, true);
            if (isConnectedOnline()) {

//                preferences = getSharedPreferences(PREFS_NAME, 0);
//                token = preferences.getString(Current_City_Code, null);
//                currentCity = preferences.getString(Current_City_Code, null);
//                currentCountry = preferences.getString(Current_Country_Code, null);

                titleText.setText("Daily forecast for " + currentCity + "," + currentCountry);

//                StringBuilder api = new StringBuilder();
//                api.append("http://dataservice.accuweather.com/locations/v1/");
//                api.append(currentCountry);
//                api.append("/search?apikey=");
//                api.append(API_KEY);
//                api.append("&q=");
//                api.append(currentCity);
                String apiUrl = ParserUtility.getLocationApiUrl(currentCity, currentCountry, getResources().getString(R.string.api_key));
                Log.d("demo", "Location api:  " + apiUrl.toString());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.save_city:
                ...
                m_Details.setCityName(currentCity);
                m_Details.setCountry(currentCountry);
...
//                    m_Details.setFavourite(true);
//                    Map<String, Object> cityValues = m_Details.toMap();
//                    Map<String, Object> childUpdates = new HashMap<>();
//                    childUpdates.put("/citydetails/" + cityKey, cityValues);
//                    mDatabaseRef.updateChildren(childUpdates);
            ...
            case R.id.set_curr_city:
                String token = preferences.getString(Current_City_Code, null);
                String mCurrentCity = preferences.getString(Current_City_Code, null);
                String mCurrentCountry = preferences.getString(Current_Country_Code, null);
                if (!mCurrentCity.isEmpty()) {
                    //Ask Nidhi
                    SharedPreferences.Editor editor;
                    editor = preferences.edit();
                    editor.putString(Current_Country_Code, currentCountry);
                    editor.putString(Current_City_Code, currentCity);
                    editor.commit();
    ...
    public void loadImages(String id, boolean isDay) {
        ...
        Log.d("demo", "Id length : " + id + ": :" + id.length());
